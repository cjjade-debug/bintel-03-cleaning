...
class CustomerDataPreparer:
    ...
    def prepare(self) -> None:
        ...
        df = self._compute_spending_score(df)
        df = self._compute_tier_level(df)
    ...
    @staticmethod
    def _compute_spending_score(df: pd.DataFrame) -> pd.DataFrame:
        """Compute SpendingScore based on spending data (adjust logic as needed)."""
        if "Spending" in df.columns:
            df["SpendingScore"] = pd.cut(
                df["Spending"],
                bins=5,
            ...
            # Fallback if spending column doesn't exist
            df["SpendingScore"] = "Unknown"
        return df

    @staticmethod
    def _compute_tier_level(df: pd.DataFrame) -> pd.DataFrame:
        """Compute TierLevel based on SpendingScore or custom criteria."""
        if "SpendingScore" in df.columns:
            ...
            df["TierLevel"] = df["SpendingScore"].map(tier_mapping)
        else:
            df["TierLevel"] = "Standard"
        return df

    @staticmethod
    def _select_columns(df: pd.DataFrame) -> pd.DataFrame:
        return df[
            ["CustomerID", "Name", "Region", "JoinDate", "SpendingScore", "TierLevel"]
        ]
